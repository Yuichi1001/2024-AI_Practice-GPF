...
def main():
    ...
    if len(val_acc_list) > 0:
        # 计算每个数组的平均值
        val_acc_means = [np.mean(val) for val in val_acc_list]
        max_index = np.argmax(val_acc_means)
        print(f"Max index in val_acc_list: {max_index}")
        print(str(np.array(val_acc_list)[max_index]))  # 打印最大值

        # 计算并打印 test_acc_easy_list 和 test_acc_hard_list 中各元素的均值
        test_acc_easy_mean = np.mean(test_acc_easy_list[max_index])
        test_acc_hard_mean = np.mean(test_acc_hard_list[max_index])
        print(f"Mean of test_acc_easy_list[{max_index}]: {test_acc_easy_mean}")
        print(f"Mean of test_acc_hard_list[{max_index}]: {test_acc_hard_mean}")

        with open('result.log', 'a+') as f:
            f.write(
                f"{str(args.runseed)} {str(test_acc_easy_list[max_index])} {str(test_acc_hard_list[max_index])}\n")
            f.write(f"Mean of test_acc_easy_list[{max_index}]: {test_acc_easy_mean}\n")
            f.write(f"Mean of test_acc_hard_list[{max_index}]: {test_acc_hard_mean}\n")
    else:
        print("Error: val_acc_list is empty. Cannot find max index.")
